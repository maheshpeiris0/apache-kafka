...
def produce_message():
    while True:
        ...
        # Use the symbol as the key for partitioning
        key = message['symbol']
        
        # Produce and send the message with the key
        producer.produce(topic, message_str.encode('utf-8'), key=key.encode('utf-8'), callback=delivery_report)
